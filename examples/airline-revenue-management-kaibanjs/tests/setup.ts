 ...
 *
 * These variables are required by RevenueManagementKaibanController.build() but are not
...
process.env.KAIBAN_REVENUE_MANAGEMENT_AGENT_ID = 'test-agent-id';
 ...
 * used by the Revenue Management Agent, allowing tests to run without
...
/**
 * Mock @kaibanjs/workflow to avoid requiring actual workflow execution in tests
 */
vi.mock('@kaibanjs/workflow', () => {
  return {
    createStep: vi.fn((config: any) => ({
      ...config,
    })),
    createWorkflow: vi.fn((config: any) => {
      const workflow = {
        ...config,
        then: vi.fn(() => workflow),
        commit: vi.fn(),
