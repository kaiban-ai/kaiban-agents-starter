 ...
 */
export const setupGroupBookingQuoteAgentRoutes = (app: express.Express, baseUrl: string) => {
  const agentPath = '/airlineGroupBookingQuote/a2a';
  const agentUrl = `${baseUrl}${agentPath}`;

  new A2AExpressApp(createGroupBookingQuoteAgentHandler(agentUrl)).setupRoutes(app, agentPath);
