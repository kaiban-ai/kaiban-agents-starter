...
const MOCK_PRICING: Record<
  string,
  { baseFarePerPax: number; discount10to19: number; discount20to49: number; discount50plus: number }
> = {
  'LHR-MIA': { baseFarePerPax: 520, discount10to19: 5, discount20to49: 10, discount50plus: 15 },
  'JFK-LAX': { baseFarePerPax: 380, discount10to19: 5, discount20to49: 10, discount50plus: 15 },
  'LHR-BCN': { baseFarePerPax: 180, discount10to19: 5, discount20to49: 10, discount50plus: 15 },
};
...
class AvailabilityTool {
  ...
  schema = z.object({
    ...
    destination: z.string().describe('Destination airport IATA code (e.g. MIA)'),
  ...
  async invoke(input: AvailabilityToolInput) {
    ...
    const inRange = input.paxCount >= routeConfig.minPax && input.paxCount <= routeConfig.maxPax;
    return JSON.stringify({
      available: inRange,
      availableSeats: inRange ? routeConfig.maxPax : 0,
      flightOptions: inRange
        ? [
            { flightNumber: 'AA100', date: input.outboundDate, seats: routeConfig.maxPax },
            input.returnDate
              ? { flightNumber: 'AA101', date: input.returnDate, seats: routeConfig.maxPax }
              : null,
          ].filter(Boolean)
        : [],
      message: inRange
...
class PricingRulesTool {
  ...
  schema = z.object({
    ...
    paxCount: z.number().describe('Number of passengers'),
    cabinClass: z.string().optional().describe('Cabin class (e.g. economy, premium, business)'),
  });
...
Output the structured inquiry plus valid, and if not valid then missingFields and validationMessage.`,
  ...
  outputSchema: z.object({
    ...
    valid: z.boolean().describe('True if all required fields are present and valid'),
    missingFields: z
      .array(z.string())
      .optional()
      .describe('List of missing or invalid field names'),
    validationMessage: z.string().optional().describe('User-friendly message when valid is false'),
  }),
});
 ...
 */
export async function createGroupBookingQuoteTeam(context: GroupBookingQuoteTeamContext) {
...
5. Return ONLY an object with one field: userMessage, set to the card description text (the inquiry). This will be used as input for the next task.`,
    ...
    expectedOutput: 'Object with userMessage set to the card description (customer inquiry text).',
    outputSchema: z.object({
...
Use the exact quote text produced by the previous task as the result to write on the card.`,
    agent: kaibanCardSyncAgent,
    expectedOutput: 'Card updated with quote result and moved to done; activities logged.',
    outputSchema: z.object({
