import { vi } from 'vitest';

/**
 * Setup test environment variables
 *
 * These variables are required by AirportServicesKaibanController.build() but are not
 * needed in tests since we mock the Kaiban SDK and AWS Bedrock.
 */
process.env.KAIBAN_TENANT = 'test-tenant';
process.env.KAIBAN_API_TOKEN = 'test-token';
process.env.KAIBAN_AIRPORT_SERVICES_AGENT_ID = 'test-agent-id';
process.env.AWS_REGION = 'us-east-1';
process.env.AWS_ACCESS_KEY_ID = 'test-access-key';
process.env.AWS_SECRET_ACCESS_KEY = 'test-secret-key';

/**
 * Mock @kaiban/sdk to avoid requiring Kaiban platform runtime in tests.
 *
 * This mock provides stub implementations of all KaibanClient methods
 * used by the Airport Services Advisor Agent, allowing tests to run without
 * connecting to a real Kaiban platform instance.
 */
vi.mock('@kaiban/sdk', () => {
  return {
    createKaibanClient: vi.fn(() => ({
      // Agent operations
      agents: {
        get: vi.fn(async (id: string) => ({
          id,
          name: 'Airport Services Advisor Agent',
          type: 'agent',
        })),
      },

      // Card operations
      cards: {
        get: vi.fn(async (id: string) => ({
          id,
          description: 'Test card description',
          column_key: 'todo',
          status: 'todo',
          board_id: 'test-board',
          team_id: 'test-team',
        })),
        update: vi.fn(async () => ({})),
        createBatchActivities: vi.fn(async () => ({})),
      },

      // Cost calculation operations
      costs: {
        calculateCosts: vi.fn((_usages: any[]) => ({
          totalCost: 0.001,
          totalTokens: 100,
          costsByModel: {
            'us.anthropic.claude-3-5-haiku-20241022-v1:0': {
              model: 'us.anthropic.claude-3-5-haiku-20241022-v1:0',
              inputTokens: 50,
              outputTokens: 50,
              totalTokens: 100,
              inputCost: 0.0005,
              outputCost: 0.0005,
              totalCost: 0.001,
            },
          },
        })),
      },
    })),

    // Export enums and types used in the code
    ActivityType: {
      CARD_CREATED: 'CARD_CREATED',
      CARD_CLONED: 'CARD_CLONED',
      CARD_AGENT_ADDED: 'CARD_AGENT_ADDED',
      CARD_COLUMN_CHANGED: 'CARD_COLUMN_CHANGED',
      CARD_STATUS_CHANGED: 'CARD_STATUS_CHANGED',
      THREAD_STEP_COSTS: 'THREAD_STEP_COSTS',
    },

    CardStatus: {
      TODO: 'todo',
      DOING: 'doing',
      DONE: 'done',
      BLOCKED: 'blocked',
    },

    A2ADataPartType: {
      KAIBAN_ACTIVITY: 'kaiban_activity',
    },
  };
});

/**
 * Mock @aws-sdk/client-bedrock-runtime to avoid requiring actual AWS API calls in tests
 */
vi.mock('@aws-sdk/client-bedrock-runtime', () => {
  const mockSend = vi.fn(async () => ({
    body: new TextEncoder().encode(
      JSON.stringify({
        content: [
          {
            type: 'text',
            text: 'Mocked response from Airport Services Advisor Agent',
          },
        ],
        usage: {
          input_tokens: 50,
          output_tokens: 50,
          reasoning_tokens: 0,
        },
      }),
    ),
  }));

  return {
    BedrockRuntimeClient: vi.fn().mockImplementation(() => ({
      send: mockSend,
    })),
    InvokeModelCommand: vi.fn().mockImplementation((input) => input),
  };
});
