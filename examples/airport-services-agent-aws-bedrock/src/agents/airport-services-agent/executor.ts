...
import { AirportServicesKaibanController } from './controller/kaiban-controller';
...
const logger = createLogger('AirportServicesAgentExecutor');

/**
 * Executor for the Airport Services Advisor Agent following the A2A (Agent-to-Agent) protocol.
 * Responsible for handling task execution lifecycle and integrating with Kaiban platform.
 *
 * @class AirportServicesAgentExecutor
 ...
 */
class AirportServicesAgentExecutor implements AgentExecutor {
  /**
   ...
   * processing to the AirportServicesKaibanController.
   */
  async execute(requestContext: RequestContext, eventBus: ExecutionEventBus): Promise<void> {
    ...
    if (userMessageData) {
      ...
      const kaibanController = await AirportServicesKaibanController.build();
  ...
  async cancelTask(taskId: string, _eventBus: ExecutionEventBus): Promise<void> {
    ...
    throw new Error('Task cancellation is not yet implemented for Airport Services Advisor Agent');
  }
}

/**
 * Singleton instance of the Airport Services Advisor Agent executor.
 * Used by the A2A handler to process incoming requests.
 *
 * @constant
 * @type {AirportServicesAgentExecutor}
 */
export const airportServicesAgentExecutor = new AirportServicesAgentExecutor();
