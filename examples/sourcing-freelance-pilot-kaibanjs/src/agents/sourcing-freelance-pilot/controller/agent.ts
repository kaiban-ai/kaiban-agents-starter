...
class ExcelDownloadTool {
  name = 'excel_download';
  description = 'Downloads and filters pilot data from Excel files accessible via URL';
  schema = z.object({
    excelUrl: z
      .string()
      .url()
      .describe('Public URL of the Excel file to download. Must be accessible via HTTP/HTTPS.'),
    filters: z
      .object({
        aircraftType: z
          .string()
    ...
    try {
      ...
      const filteredPilots = pilots.filter((pilot) => {
        ...
        if (filters.pilotRole) {
          const roleMatch = pilot.function.toUpperCase().includes(filters.pilotRole.toUpperCase());
          if (!roleMatch) return false;
        }

        // San Marino validation filter
        if (filters.sanMarinoValidation === true) {
          if (!pilot.sanMarinoValidation || pilot.sanMarinoValidation.toUpperCase() !== 'YES') {
            return false;
          }
        }
        ...
        if (filters.licenseType) {
          if (
            !pilot.licence ||
            !pilot.licence.toUpperCase().includes(filters.licenseType.toUpperCase())
          ) {
            return false;
          }
        }
        ...
        // Gender filter
        if (filters.gender) {
          const genderMatch = pilot.gender.toLowerCase().includes(filters.gender.toLowerCase());
...
CRITICAL: Extract ALL filtering parameters in this task so the next task can directly use them without re-analysis.`,
  ...
  outputSchema: z.object({
    ...
    filters: z
      .object({
        aircraftType: z.enum(['GLEX - Global Express XRS (BD-700)', 'Cessna Citation M2']),
      ...
      .optional()
      .describe('Filtering parameters extracted from flight request and special rules'),
    aircraftTypes: z
      .array(
        z.object({
  ...
  outputSchema: z.object({
    candidates: z.array(
      z.object({
        ...
        processingStatus: z.enum(['initial', 'message_sent', 'response_received', 'completed']),
      }),
    ),
    ...
    flightRequest: z.string().describe('Flight request text for message templates'),
    specialRules: z.string().optional().describe('Special rules text for message templates'),
  }),
});
...
export const createPilotSourcingTeam = (userMessage: string) => {
  return new Team({
    ...
    agents: [textExtractionAgent, pilotQualificationAgent, messageTemplateAgent],
    tasks: [extractParamsAndFiltersTask, processCandidatesTask, createMessagesTemplateTask],
    inputs: {
      userMessage,
    },
    env: {
