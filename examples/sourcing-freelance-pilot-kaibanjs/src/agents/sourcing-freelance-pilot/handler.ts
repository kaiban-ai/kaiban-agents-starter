 ...
 * ```
 */
export const setupPilotSourcingAgentRoutes = (app: express.Express, baseUrl: string) => {
  const agentPath = '/pilotSourcing/a2a';
  const agentUrl = `${baseUrl}${agentPath}`;

  // Setup A2A routes
  new A2AExpressApp(createPilotSourcingAgentHandler(agentUrl)).setupRoutes(app, agentPath);
