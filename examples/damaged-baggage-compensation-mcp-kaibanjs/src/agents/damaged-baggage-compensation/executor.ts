/**
 * @fileoverview A2A Executor for Damaged Baggage Compensation Agent
 *
 * Processes incoming A2A requests, extracts Kaiban activities, and runs the
 * Damaged Baggage Compensation Team. Card lifecycle is handled via Kaiban MCP.
 * On team error, the executor moves the card to blocked.
 *
 * @module agents/damaged-baggage-compensation/executor
 */
...
import { processDamagedBaggageCompensationRequest } from './controller/agent';
import { getCard, moveCardToBlocked } from './controller/kaiban-mcp-client';

export const tasksStore = new InMemoryTaskStore();

const logger = createLogger('DamagedBaggageCompensationExecutor');

const OUR_AGENT_NAME = 'Damaged Baggage Compensation Agent';

function getOurAgentId(): string | null {
  return (
    process.env.KAIBAN_DAMAGED_BAGGAGE_COMPENSATION_AGENT_ID || process.env.KAIBAN_AGENT_ID || null
  );
}

class DamagedBaggageCompensationAgentExecutor implements AgentExecutor {
  async execute(requestContext: RequestContext, eventBus: ExecutionEventBus): Promise<void> {
    ...
    if (userMessageData?.length && ourAgentId) {
      eventBus.publish({
        kind: 'status-update',
        taskId,
        contextId,
      ...
      for (const messageData of userMessageData) {
        ...
        if (activity.actor?.id === ourAgentId) {
          logger.debug({ cardId: activity.card_id }, 'Skipping activity triggered by this agent');
          continue;
        }
        ...
        if (!card?.description || card.column_key !== 'todo') {
          logger.debug(
            { cardId, hasDescription: !!card?.description, column_key: card?.column_key },
        ...
        try {
          ...
          logger.error(
            ...
            'Failed to process card with damaged baggage compensation team',
          );
          try {
            ...
            logger.error({ blockedErr, cardId }, 'Failed to move card to blocked via MCP');
          }
        }
      }
    }
  ...
  async cancelTask(taskId: string, _eventBus: ExecutionEventBus): Promise<void> {
    ...
    throw new Error(
      'Task cancellation is not yet implemented for Damaged Baggage Compensation Agent',
    );
  }
}

export const damagedBaggageCompensationExecutor = new DamagedBaggageCompensationAgentExecutor();
