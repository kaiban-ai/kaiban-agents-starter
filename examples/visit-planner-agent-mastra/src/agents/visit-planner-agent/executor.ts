...
import { KaibanController } from './controller/kaiban-controller';
...
const logger = createLogger('VisitPlannerAgentExecutor');

/**
 * Executor for the Visit Planner Agent following the A2A (Agent-to-Agent) protocol.
 * Responsible for handling task execution lifecycle and integrating with Kaiban platform.
 *
 * @class VisitPlannerAgentExecutor
 ...
 */
class VisitPlannerAgentExecutor implements AgentExecutor {
  /**
   ...
   * processing to the KaibanController.
   */
  async execute(requestContext: RequestContext, eventBus: ExecutionEventBus): Promise<void> {
    ...
    if (userMessageData) {
      ...
      const kaibanController = await KaibanController.build();
  ...
  async cancelTask(taskId: string, _eventBus: ExecutionEventBus): Promise<void> {
    ...
    throw new Error('Task cancellation is not yet implemented for Visit Planner Agent');
  }
}

/**
 * Singleton instance of the Visit Planner Agent executor.
 * Used by the A2A handler to process incoming requests.
 *
 * @constant
 * @type {VisitPlannerAgentExecutor}
 */
export const visitPlannerAgentExecutor = new VisitPlannerAgentExecutor();
